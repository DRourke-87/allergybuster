...
PollenFetchWorker.doWork()
  ...
  6. NotificationHelper.postDailyNotification() + postPersistentNotification()
```
...
- View models conform to `ObservableObject`, observe shared repos via Swift `AsyncSequence`
  (bridged by SKIE).
- `BackgroundRefreshScheduler` handles `BGAppRefreshTask`; an immediate fetch is also
  kicked off at launch so the home screen has data without waiting for the scheduled task.
- The App Group (`group.com.tarnlabs.allergybuster`) backs `UserDefaults` used for the
  learning started-at timestamp, notification time and location persistence.
- iOS builds do **not** run Gradle; they consume `shared.xcframework` generated by
  `./gradlew :shared:assembleXCFramework`.
- **App icon**: the Xcode project requires `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon`
  on the app target (both Debug and Release) for the icon to be included in builds.
  Without it the icon slot is empty on device even though the asset exists in xcassets.
- **Onboarding**: `OnboardingView` has three pages. Tapping "Get started" on the final page
  triggers the system location permission dialog before dismissing onboarding.
  `SettingsViewModel` handles the grant callback and kicks off the first location fetch.
